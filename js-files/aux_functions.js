function FindPosition(position, n) {
    let j = 0;
...
function IntArrayToString(input) {
...
function LoadData(needGenerateSalt) {
    ...
    var info = {
        ...
        salt: (document.getElementById('flexSwitchCheckDefault').checked)? true: false,
...
function SaltCharGen(needGenerateSalt) {

    if (needGenerateSalt == false) {
        return "";
    }
    else {
        return SaltGenerator(4).join('');
    }
...
function MakeShufflePositionArr(hash, messageLength) {
    ...
    const FULL_ARR_SIZE = messageLength > 5000? Math.floor(Math.sqrt(messageLength)):250; //Math.sqrt(messageLength)
...
function SaltGenerator(saltLenght) {
    var numberR;
    let saltArray = [];
...
function Average(e) {
    ...
    return Math.floor(sum / e.length);
}
...
function HashComplex(originalHash, complexity) {
    ...
    for (let i = 0; i < 10 ** complexity; i++) {

        hashOutput = String(sha512.create().update(hashOutput).hex());
    } return hashOutput;   

    /*
    //Recursive version HashComplex(originalHash, complexity, n)
    //problem: Maximum call stack size exceeded error

    if (n == ((10 ** complexity))) return String(originalHash);
    else {
        let input = String(sha512.create().update(originalHash).hex() );
        return HashComplex(input, complexity, n + 1);
    }
    */
...
function ConvertCharArrayIntoInt(sArray) {
    let intArray = [];

    for (let j = 0; j < sArray.length; j++) {
        intArray[j] = (sArray.charAt(j)).charCodeAt(0);
        j++;
    };
